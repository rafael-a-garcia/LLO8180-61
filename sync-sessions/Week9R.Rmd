...
#install.packages("effectsize")#install.packages("tidyverse")
```

This week we will learn how to conduct a one-way ANOVA in R. 

We are going to use the depression data for our example. We will import using the 
working directory (doing it this way should help some of the knitting issues)
```{r}
#first, set working directory (session -> set working directory -> choose directory)
#make sure RMD file and dataset is in the folder you choose for the working directory
#load the depression data
dep_data <- read.table("Depression.txt", header=T, sep="\t")
```
...
```{r}
#We're going to use Tukey's HSD for our post hoc analysis
TukeyHSD(anova1)
```
